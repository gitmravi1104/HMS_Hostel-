...
const HostelerHome = () => {
  ...
  const {setDataCon} = useContext(HMContextCreater)
  ...
  ? hostelerData.filter(data =>
      (data.hostlerName && data.hostlerName.toLowerCase().includes(trimmedSearchTerm)) || 
      (data.hostlerMobile && data.hostlerMobile.toLowerCase().includes(trimmedSearchTerm)) || 
      (data.hostlerRoomNo && data.hostlerRoomNo.toLowerCase().includes(trimmedSearchTerm))
    )
  : hostelerData;
            ...
            {!notFound && filteredData.length > 0 && (
                filteredData.map((item,index)=>(
                  <tr >
                    ...
                    <td><button className='rounded-circle' onClick={()=>HandleUpdateHosteler(item)}  ><i className="fa-solid fa-ellipsis text-primary"></i></button></td>
                  </tr>
                ))
              )}
